...
#ifndef TINY_CV_OPENCV_H
#define TINY_CV_OPENCV_H
...
struct Mat
{
    ...
    void create(int _rows, int _cols, int flags)
    {
        ...
        if (total() > 0)
        {
            ...
            data = (unsigned char*)tiny_cv::fastMalloc(totalsize + (int)sizeof(*refcount));
    ...
    void release()
    {
        if (refcount && TINY_CV_XADD(refcount, -1) == 1)
            tiny_cv::fastFree(data);
